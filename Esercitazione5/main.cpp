...
#define TEMP_IN 18.                     // Initial temperature
#define SIGMA_VELOCITIES sqrt(TEMP_IN)  // Sigma Velocities
#define N_PARTICLE 300                 // Number of particles in simulation
#define SIGMA 1.                        // Distance scale
#define EPSILON 1.                      // Energy scale
#define DENSITY 0.1                     // Density of particle
#define MASS 1.                         // Particle mass
#define VOLUME (N_PARTICLE / DENSITY)   // Volume a single cell (Volume in unit of sigma^3)
...
double particlesSquareDistance(Particle *p1, Particle *p2) {
...
Vec3 particlesVersor(Particle *p1, Particle *p2) {
...
Vec3 *getForcesLennarJones(System *system, double *args) {
    ...
    for (size_t i = 0; i < system->N_particles - 1; i++) {
        for (size_t j = i + 1; j < system->N_particles; j++) {
            ...
            Vec3 r_dir = particlesVersor(&system->particles[i], &system->particles[j]);
            double F_magnitude = 24 * EPSILON * SIGMA / r_square * (2 * pow(SIGMA / r, 11) - pow(SIGMA / r, 5));
            Vec3 force_ij = {r_dir.x * F_magnitude, r_dir.y * F_magnitude, r_dir.z * F_magnitude};
...
double var_array(double *array, int len) {
...
void printCMVelocity(System *system, FILE *file = stdout) {
